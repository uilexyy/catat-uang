    ...
    <div className="flex flex-col h-[500px] rounded-2xl overflow-hidden bg-[#efeae2] dark:bg-stone-800 border border-stone-200 dark:border-stone-700 relative">
      ...
      >
        ...
        <div className="flex justify-center mb-2">
          <span className="text-[11px] text-stone-400 dark:text-stone-500 bg-white/70 dark:bg-stone-800/70 px-3 py-1 rounded-full shadow-xs">
            Hari ini
          </span>
        </div>
        ...
        {messages.map((msg, i) => (
          ...
          >
            {msg.role === "bot" && (
              <div
                className={`w-7 h-7 rounded-full flex items-center justify-center shrink-0 self-end mb-1 ${
                  msg.error ? "bg-rose-100 dark:bg-rose-950/50" : "bg-blue-100 dark:bg-blue-950/50"
                }`}
              >
                {msg.error ? (
            ...
            <div className="max-w-[80%] group relative">
              ...
              <p
                className={`text-[10px] text-stone-400 dark:text-stone-500 mt-0.5 px-1 ${
          ...
          <div className="flex gap-2 px-1 mt-1 animate-fade-in">
            ...
            <div className="bg-white dark:bg-stone-900 rounded-[18px] rounded-bl-[4px] px-4 py-3 shadow-sm border border-stone-100 dark:border-stone-800">
              <div className="flex gap-1">
                ...
                <span className="w-2 h-2 rounded-full bg-stone-300 dark:bg-stone-600 animate-bounce" style={{ animationDelay: "0.3s" }} />
              </div>
            </div>
          </div>
        )}
          ...
          <div className="flex flex-wrap gap-1.5 justify-center mt-3">
            {suggestions.map((s) => (
              <button
                ...
                className="text-[11px] px-3 py-1.5 rounded-full bg-white/80 dark:bg-stone-800/80 text-stone-500 dark:text-stone-400 hover:text-blue-600 hover:bg-white dark:hover:bg-stone-800 border border-stone-200 dark:border-stone-700 hover:border-blue-200 shadow-xs transition-all duration-200 active:scale-95"
              >
                {s}
              </button>
            ))}
      ...
      {/* Input area */}
      <div className="shrink-0 bg-[#efeae2] dark:bg-stone-800 px-3 py-2.5">
        <div className="flex items-center gap-2 bg-white dark:bg-stone-900 rounded-2xl border border-stone-200 dark:border-stone-700 pl-4 pr-2 py-1.5 shadow-xs focus-within:border-blue-400 focus-within:ring-2 focus-within:ring-blue-500/20 transition-all duration-200">
          <input
            ref={inputRef}
            type="text"
            value={input}
            ...
            disabled={loading}
            className="flex-1 text-sm text-stone-700 dark:text-stone-300 placeholder-stone-300 dark:placeholder-stone-600 bg-transparent focus:outline-none disabled:opacity-50"
          />
          <button
            type="button"
