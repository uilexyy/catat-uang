...
export default function Navbar() {
  ...
  return (
    <>
      {/* Desktop: Sidebar */}
      <aside className="hidden md:flex flex-col w-56 h-screen sticky top-0 bg-white border-r border-stone-200/60 shrink-0">
        <div className="px-5 pt-5 pb-6">
          <Link href="/" className="flex items-center gap-2.5 group">
            ...
            <span className="font-bold text-lg text-stone-800 group-hover:text-blue-600 transition-colors duration-300">
              Catat Uang
            </span>
          </Link>
        </div>
        ...
        <div className="px-5 py-4 border-t border-stone-100">
          <p className="text-[11px] text-stone-300">Catat Uang v1.0</p>
        </div>
      </aside>

      {/* Mobile: Bottom Nav */}
      <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-stone-200 z-50 pb-[env(safe-area-inset-bottom,0px)]">
            ...
            return (
              <Link
                ...
                className={`flex flex-col items-center justify-center gap-0.5 min-w-14 px-2 h-full transition-all duration-200 relative ${
                  isActive ? "text-blue-600" : "text-stone-400 hover:text-stone-500"
                }`}
              >
                {isActive && (
